...
-include("mnesia_sqlite_tuning.hrl").
...
load_tuning_parameters() ->
    case application:get_env(mnesia_sqlite, tuning_params) of
        {ok, Ps} ->
            case Ps of
                {consult, F} -> consult(F);
                {script, F} -> script(F);
...
store_params(Params) ->
    ...
    NTabs = length(Params),
    Env0= mnesia_sqlite_tuning:describe_env(),
    Env = Env0#tuning{n_tabs = NTabs},
    ?dbg("Env = ~p~n", [Env]),
    TotalFiles = lists:sum([mnesia_sqlite_tuning:max_files(Sz) ||
                               {_, Sz} <- Params]),
...
rdb_params(Sz, Env, _Tab) ->
    MaxFiles = mnesia_sqlite_tuning:max_files(Sz) * Env#tuning.files_factor,
    Opts = if Env#tuning.avail_ram > 100 ->  % Gigabytes
                 [{write_buffer_size, mnesia_sqlite_tuning:write_buffer(Sz)},
                  {cache_size, mnesia_sqlite_tuning:cache(Sz)}];
              true ->
                   []
           end,
