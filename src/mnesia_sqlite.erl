...
%% @doc sqlite storage backend for Mnesia.

%% Initialization: register() or register(Alias)
%% Usage: mnesia:create_table(Tab, [{sqlitedb_copies, Nodes}, ...]).

-module(mnesia_sqlite).
...
%% Name of the Sqlite interface module; defaults to sqlite but can be
%% configured by passing -DSQLITE_MODULE=<name_of_module> to erlc.
-ifdef(SQLITE_MODULE).
-define(sqlite, ?SQLITE_MODULE).
-else.
-define(sqlite, sqlite_mod). %% Name of the Sqlite interface module
-endif.
...
%% A debug function that shows the sqlite table content
show_table(Tab) ->
...
i_show_table(I, Move, Limit) ->
    case ?sqlite:iterator_move(I, Move) of
        {ok, EncKey, EncVal} ->
            {Type,Val} =
                case EncKey of
                    << ?INFO_TAG, K/binary >> ->
...
init_backend() ->
    stick_sqlitedb_dir(),
    {ok, _} = application:ensure_all_started(mnesia_sqlite),
    ok.

%% Prevent reloading of modules in sqlite itself during runtime, since it
%% can lead to inconsistent state in sqlite and silent data corruption.
stick_sqlitedb_dir() ->
    case code:which(sqlite) of
...
warn_stick_dir(Reason) ->
    mnesia_lib:warning("cannot make sqlite directory sticky:~n~p~n",
                       [Reason]).
...
check_definition(Alias, Tab, Nodes, Props) ->
    ...
    Props1 = lists:map(
               fun({type, T} = P) ->
                  ...
                  ({user_properties, _} = P) ->
                       %% should perhaps verify sqlite options...
                       P;
                  (P) -> P
               end, Props),
    {ok, Props1}.
...
load_table(Alias, Tab, _LoadReason, Opts) ->
    ...
    RdbUserProps = proplists:get_value(
                     sqlitedb_opts, proplists:get_value(
                                     user_properties, Opts, []), []),
    StorageProps = proplists:get_value(
                     sqlite, proplists:get_value(
                                storage_properties, Opts, []), RdbUserProps),
    RdbOpts = mnesia_sqlite_params:lookup(Tab, StorageProps),
...
load_table_(Alias, Tab, Type, RdbOpts) ->
    ...
    case mnesia_ext_sup:start_proc(
        ...
        {error, {already_started, _Pid}} ->
            ...
            %% transform_table on a sqlitedb_table that has indexing.
...
%% PRIVATE ITERATOR
i_first(I) ->
    case ?sqlite:iterator_move(I, <<?DATA_START>>) of
    {ok, First, _} ->
        decode_key(First);
    _ ->
        '$end_of_table'
...
%% PRIVATE ITERATOR
i_last(I) ->
    case ?sqlite:iterator_move(I, last) of
...
lookup_bag(Ref, K, Enc, KP) ->
    ...
    with_iterator(
      Ref, fun(I) ->
           lookup_bag_(Sz, Enc, ?sqlite:iterator_move(I, Enc),
                   K, I, KP)
       end).

lookup_bag_(Sz, Enc, {ok, Enc, _}, K, I, KP) ->
    lookup_bag_(Sz, Enc, ?sqlite:iterator_move(I, next), K, I, KP);
...
%% PRIVATE ITERATOR
i_next(I, EncKey, Key) ->
    case ?sqlite:iterator_move(I, EncKey) of
        {ok, EncKey, _} ->
            i_next_loop(?sqlite:iterator_move(I, next), I, Key);
        Other ->
...
%% PRIVATE ITERATOR
i_prev(I, Key) ->
    case ?sqlite:iterator_move(I, Key) of
    {ok, _, _} ->
...
%% PRIVATE ITERATOR
i_move_to_prev(I, Key) ->
    case ?sqlite:iterator_move(I, prev) of
    {ok, << ?INFO_TAG, _/binary >>, _} ->
        '$end_of_table';
...
slot(Alias, Tab, Pos) when is_integer(Pos), Pos >= 0 ->
    {Ref, Type} = get_ref(Alias, Tab),
    First = fun(I) -> ?sqlite:iterator_move(I, <<?DATA_START>>) end,
    F = case Type of
...
slot_iter_set({ok, _, _}, I, P1, P) when P1 < P ->
    slot_iter_set(?sqlite:iterator_move(I, next), I, P1+1, P);
...
do_update_counter(C, Val, Ref) ->
    Enc = encode_key(C),
    case ?sqlite:get(Ref, Enc, [{fill_cache, true}]) of
    {ok, EncVal} ->
...
with_iterator(Ref, F) ->
    {ok, I} = ?sqlite:iterator(Ref, []),
    try F(I)
    after
        ?sqlite:iterator_close(I)
    end.

%% keys_only iterator: iterator_move/2 returns {ok, EncKey}
%% with_keys_only_iterator(Ref, F) ->
%%     {ok, I} = ?sqlite:iterator(Ref, [], keys_only),
%%     try F(I)
%%     after
%%         ?sqlite:iterator_close(I)
%%     end.
...
do_load_table(Tab, RdbOpts) ->
    ...
    {ok, Ref} = open_sqlitedb(MPd, RdbOpts),
    sqlitedb_to_ets(Ref, Ets),
    {ok, Ref, Ets}.
...
handle_call(clear_table, _From, #st{ets = Ets, tab = Tab, ref = Ref} = St) ->
    ...
    _ = sqlitedb_close(Ref),
    {ok, NewRef} = destroy_recreate(MPd, sqlitedb_open_opts(Tab)),
    ets:delete_all_objects(Ets),
    sqlitedb_to_ets(NewRef, Ets),
...
handle_info({'EXIT', _, _} = _EXIT, St) ->
    ?dbg("sqlite owner received ~p~n", [_EXIT]),
    {noreply, St};
handle_info(_, St) ->
    {noreply, St}.
...
sqlitedb_open_opts({Tab, index, {Pos,_}}) ->
    UserProps = mnesia_lib:val({Tab, user_properties}),
    IxOpts = proplists:get_value(sqlitedb_index_opts, UserProps, []),
    PosOpts = proplists:get_value(Pos, IxOpts, []),
    sqlitedb_open_opts_(PosOpts);
sqlitedb_open_opts(Tab) ->
    UserProps = mnesia_lib:val({Tab, user_properties}),
    RdbOpts = proplists:get_value(sqlitedb_opts, UserProps, []),
    sqlitedb_open_opts_(RdbOpts).
...
default_open_opts() ->
    [ {create_if_missing, true}
      , {cache_size,
         list_to_integer(get_env_default("SQLITE_CACHE_SIZE", "32212254"))}
      , {block_size, 1024}
      , {max_open_files, 100}
      , {write_buffer_size,
         list_to_integer(get_env_default(
                           "SQLITE_WRITE_BUFFER_SIZE", "4194304"))}
      , {compression,
         list_to_atom(get_env_default("SQLITE_COMPRESSION", "true"))}
      , {use_bloomfilter, true}
    ].

destroy_recreate(MPd, RdbOpts) ->
    ok = destroy_db(MPd, []),
    open_sqlitedb(MPd, RdbOpts).

open_sqlitedb(MPd, RdbOpts) ->
    open_sqlitedb(MPd, sqlitedb_open_opts_(RdbOpts), get_retries()).

%% Code adapted from basho/riak_kv_eleveldb_backend.erl
open_sqlitedb(MPd, Opts, Retries) ->
    open_db(MPd, Opts, max(1, Retries), undefined).

open_db(_, _, 0, LastError) ->
    {error, LastError};
open_db(MPd, Opts, RetriesLeft, _) ->
    case ?sqlite:open(MPd, Opts) of
        {ok, Ref} ->
            ?dbg("~p: Open - Sqlite: ~s~n  -> {ok, ~p}~n",
                 [self(), MPd, Ref]),
            {ok, Ref};
        %% Check specifically for lock error, this can be caused if
        %% a crashed mnesia takes some time to flush sqlite information
        ...
        {error, {db_open, OpenErr}=Reason} ->
            case lists:prefix("IO error: lock ", OpenErr) of
                true ->
                    ...
                    ?dbg("~p: Open - Sqlite backend retrying ~p in ~p ms"
                         " after error ~s\n",
...
sqlitedb_close(undefined) ->
    ok;
sqlitedb_close(Ref) ->
    Res = ?sqlite:close(Ref),
...
destroy_db(MPd, Opts, RetriesLeft, _) ->
    case ?sqlite:destroy(MPd, Opts) of
    ok ->
        ok;
        %% Check specifically for lock error, this can be caused if
        ...
        {error, {error_db_destroy, Err}=Reason} ->
            case lists:prefix("IO error: lock ", Err) of
                true ->
                    SleepFor = get_retry_delay(),
                    ?dbg("~p: Destroy - Sqlite backend retrying ~p in ~p ms"
                         " after error ~s\n"
                         " children = ~p~n",
...
do_insert(K, V, #st{ets = Ets, ref = Ref, maintain_size = true}) ->
    ...
    true ->
        ...
        ?sqlite:write(Ref, [{put, Ki, Vi}, {put, K, V}], []),
        ets_insert_info(Ets, size, NewSz);
    false ->
        ?sqlite:put(Ref, K, V, [])
    end,
...
do_insert_bag(Ref, K, V, CurSz) ->
    ...
    with_iterator(
      Ref, fun(I) ->
           do_insert_bag_(
             KSz, K, ?sqlite:iterator_move(I, K), I, V, 0, Ref, CurSz)
       end).
...
do_insert_bag_(Sz, K, Res, I, V, Prev, Ref, TSz) when Prev < ?MAX_BAG ->
    ...
    {ok, <<K:Sz/binary, N:?BAG_CNT>>, _} ->
        do_insert_bag_(
          Sz, K, ?sqlite:iterator_move(I, next), I, V, N, Ref, TSz);
    _ when TSz =:= false ->
        Key = <<K/binary, (Prev+1):?BAG_CNT>>,
        ?sqlite:put(Ref, Key, V, []);
    _ ->
        NewSz = TSz + 1,
        {Ki, Vi} = info_obj(size, NewSz),
        Key = <<K/binary, (Prev+1):?BAG_CNT>>,
        ?sqlite:write(Ref, [{put, Ki, Vi}, {put, Key, V}], []),
        NewSz
    end.
...
do_delete(Key, #st{ref = Ref, maintain_size = false}) ->
    ?sqlite:delete(Ref, Key, []);
do_delete(Key, #st{ets = Ets, ref = Ref, maintain_size = true}) ->
    CurSz = read_info(size, 0, Ets),
    case ?sqlite:get(Ref, Key, [{fill_cache,true}]) of
    {ok, _} ->
        NewSz = CurSz -1,
        {Ki, Vi} = info_obj(size, NewSz),
        ok = ?sqlite:write(Ref, [{delete, Key}, {put, Ki, Vi}], []),
        ets_insert_info(Ets, size, NewSz);
    not_found ->
        false
    end.
...
do_delete_bag(Sz, Key, Ref, TSz) ->
    ...
    with_iterator(
      Ref, fun(I) ->
               do_delete_bag_(Sz, Key, ?sqlite:iterator_move(I, Key),
                      Ref, I)
           end),
    case {Found, TSz} of
    {[], _} ->
        TSz;
    {_, false} ->
        ?sqlite:write(Ref, [{delete, K} || K <- Found], []);
    {_, _} ->
        N = length(Found),
        NewSz = TSz - N,
        {Ki, Vi} = info_obj(size, NewSz),
        ?sqlite:write(Ref, [{put, Ki, Vi} |
                 [{delete, K} || K <- Found]], []),
        NewSz
    end.
...
do_delete_bag_(Sz, K, Res, Ref, I) ->
    case Res of
    {ok, K, _} ->
        do_delete_bag_(Sz, K, ?sqlite:iterator_move(I, next),
               Ref, I);
...
do_match_delete(Pat, #st{ets = Ets, ref = Ref, tab = Tab, type = Type,
    ...
    {[], _} ->
        ok;
    {_, false} ->
        ?sqlite:write(Ref, [{delete, K} || K <- Keys], []),
        ok;
    {_, true} ->
        CurSz = read_info(size, 0, Ets),
...
recover_size_info(#st{ ref = Ref
    ...
    false ->
        ...
        true ->
            %% info initialized by sqlitedb_to_ets/2
...
write_info_(Item, Val, #st{ets = Ets, ref = Ref}) ->
    sqlitedb_insert_info(Ref, Item, Val),
...
sqlitedb_insert_info(Ref, Item, Val) ->
    ...
    ?sqlite:put(Ref, EncKey, EncVal, []).

sqlitedb_delete_info(Ref, Item) ->
    EncKey = info_key(Item),
    ?sqlite:delete(Ref, EncKey, []).
...
i_do_select(I, #sel{keypat = Pfx,
    ...
    select_traverse(?sqlite:iterator_move(I, StartKey), Limit,
...
select_traverse({ok, K, V}, Limit, Pfx, MS, I, #sel{tab = Tab} = Sel,
    ...
    true ->
        ...
        [] ->
            select_traverse(
              ?sqlite:iterator_move(I, next), Limit, Pfx, MS,
              I, Sel, AccKeys, Acc);
        [Match] ->
                    Acc1 = if AccKeys ->
                                   [{K, Match}|Acc];
...
traverse_continue(_K, Limit, Pfx, MS, I, Sel, AccKeys, Acc) ->
    select_traverse(?sqlite:iterator_move(I, next), Limit, Pfx, MS, I, Sel, AccKeys, Acc).

iterator_next(I, K) ->
    case ?sqlite:iterator_move(I, K) of
    {ok, K, _} ->
        ?sqlite:iterator_move(I, next);
    Other ->
        Other
    end.
